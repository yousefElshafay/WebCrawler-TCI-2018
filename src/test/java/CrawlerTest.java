...
//    /* this method is to test whether a exception is thrown or not when the results are empty strings*/
   ...
   Get all information about mediaDataSearch Tests
    ...
    @Test(expected =InternalServerErrorException.class )
    public void CrawlerServiceGetItemsDataThrowsExceptionWhenResultsAreEmptyStrings() throws IOException{

        //arrange
        when(iSerializer.MediaDataToJSON(iCrawler.GetItemData(Correct_ID))).thenReturn(Emp_String);
        //act
        service.getDataItemService(Correct_ID);
    }
    ...
    public void GetItemsUrlShouldThrowsNullPointerException() throws IOException{
        //arrange
        when(pagesScrapper.getItemsURL(URL)).thenReturn(null);
        //act
        crawler.GetAllContents();
